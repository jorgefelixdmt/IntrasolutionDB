...
CREATE FUNCTION [dbo].[uf_ultimo_valor_tabla_techbuilder] (@pm_sequence_table_id numeric(10,0))    
...
set @id_sequence_table = (select         
  ...
  if (@id_sequence_table = 54) --si es sc_module_id    
   begin    
    set @valor_id =(select  top 1 sc_module_id  from sc_module where SC_SUPER_MODULE_ID <>1 order by 1 desc )    
   end     
