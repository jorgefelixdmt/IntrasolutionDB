...
pr_inc_aviso_incidencia_rechazada_qa_dmt 1
****************************************************************************************************************************************
RESUMEN DE CAMBIOS
...
**************************************************************************************************************************************
*/
CREATE PROCEDURE [dbo].[pr_inc_aviso_incidencia_rechazada_qa_dmt] --4,1
...
SET @CODIGO_EMAIL = 'INC_AVISO_INCIDENCIA_RECHAZADA_QA_DMT'
...
-- Obtención de responsables de corregir incidencia
SET @destinatarios_email = @destinatarios_email + ';' + @correo_responsable


-- Obtención de observaciones y adjuntos
DECLARE @observaciones varchar(500), @attachments VARCHAR(8000), @path_upload varchar(500)
...
SELECT TOP 1 
	...
	@attachments = @path_upload + archivo
FROM inc_incidencia_seguimiento
WHERE 
	is_deleted = 0 AND
	inc_estado_incidencia_id = 1 AND -- Solicitado
