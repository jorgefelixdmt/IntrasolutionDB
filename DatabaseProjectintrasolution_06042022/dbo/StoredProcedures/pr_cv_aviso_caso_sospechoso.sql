...
Descripcion: Envía un correo indicando el empleado al que se le ha registrado un síntoma e COVID-19

pr_cv_aviso_caso_sospechoso 1
...
CREATE PROCEDURE [dbo].[pr_cv_aviso_caso_sospechoso]
	@cv_declaracion_sintomas_id numeric(10,0)
As
...
SET @CODIGO_EMAIL = 'CV_AVISO_CASO_SOSPECHOSO' 
...
WHERE 
	d.cv_declaracion_id = (SELECT cv_declaracion_id FROM cv_declaracion_sintomas WHERE cv_declaracion_sintomas_id = @cv_declaracion_sintomas_id)
