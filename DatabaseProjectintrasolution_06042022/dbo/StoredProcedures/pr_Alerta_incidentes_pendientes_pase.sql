/*
*********************************************************************************************************************
Nombre:[dbo].[pr_Alerta_incidentes_pendientes]
Fecha de creación: 23/04/2021
Autor:Jorge Felix
Descripción: Alerta que envia un email por cada proyecto de los pases pendientes a procesar por parte del Cliente 
			 Los Destinarios son los responsables de los proyectos (Email Responsable de Incidente)
			 Se detallan las incidencias asociadas a cada pase
			 Se propone enviar semanalmente los lunes a primera hora 
*********************************************************************************************************************
Resumen de cambios 
Fecha(aaaa-mm-dd)	Autor			  Comentarios
-----------------	---------------	  -------------
04/05/2021          Valky Salinas     Se modificó el query para encontrar el estado siguiente del pase.
...
As
	...
	set dateformat dmy
	DECLARE @CODIGO_EMAIL varchar(150), @asunto_email varchar(1024), @cuerpo_email varchar(max), @DiaActual numeric(10,0), @Top_html varchar(max), @PASE numeric(10,0)
	DECLARE @CodigoTicket varchar(150), @Fecha varchar(10), @ReportadoPor varchar(150), @Descripcion varchar(800), @NombreCompleto varchar(150),
	...
	DECLARE @AmbitoIncidencia varchar(64), @TipoIncidencia varchar(64)   -- 2021-10-16
	...
	DECLARE @Estado varchar(150), @FechaEnvioPase varchar(10), @EstadoSiguiente varchar(150)
	DECLARE @PROY numeric(10,0), @pry_proyecto_id numeric(10,0),@pa_pase_incidente_id numeric(10,0)
	DECLARE @HTML_PASE varchar(max),@BODY varchar(max), @asunto_email_proyecto  varchar(1024)
	DECLARE  @Email varchar(250),@Email_Alerta varchar(250), @Email_Cliente_Pase varchar(450)
	...
	DECLARE Cursor_Pase CURSOR FOR
		SELECT 
			...
			(Select top 1 convert(varchar(10),IsNull(pa.fecha_solicitud,''),120) 
			...
			(Select top 1 nombre 
				...
				Where pae2.orden >  pae.orden) 
			as Estado_Siguiente,
			IsNull(email_cliente_incidente,'') as email_cliente_pase,
			pry.pry_proyecto_id,
			pa.pa_pase_id
		FROM pa_pase pa
	...
	IF @FechaEnvioPase Is Null 
		Set @FechaEnvioPase = ''

	SET @Top_html = '<style>'
	SET @Top_html = @Top_html + 'body{font-family:Arial; font-size:10}'
		...
		BEGIN	-- ***  INICIO BUCLE CURSOR_PASE				 			
			...
			IF @PROY <> @pry_proyecto_id	
				BEGIN -- Inicializa variables por PROYECTO
					...
					SET @Email = @Email_Cliente_Pase + ';' + @Email_Alerta
					SET @asunto_email_proyecto = @asunto_email + ' - ' + @CodigoProyecto
					SET @BODY = ''
				END
			...
			IF (@PROY <> @pry_proyecto_id)
				BEGIN
					...
					-- se envia el email  
					Exec pr_EMAIL_ENVIO_V2 '',@email,@asunto_email_proyecto,'HTML',@BODY ,'',@CODIGO_EMAIL,1,NULL,NULL
					-- INICIALIZA HTML
					SET @HTML_PASE = ''
				END
